...
**Band rhythm.** `.section` sets `padding-block: clamp(4rem, 9vw, 8rem)`; `.section--tight` sets `clamp(3rem, 6vw, 6rem)`. The hero runs at `clamp(3rem, 6vw, 6rem)` — tighter than a section band, not wider, so the quote form under it clears the fold, and `PageHead` runs deliberately tighter at `clamp(2rem, 4.5vw, 4rem)` — a nameplate, not a hero. Bands never carry a top and bottom border at once — the ground change is the separation. The hero and the quote form are the exception in the other direction: same ground, no border between them, joined into one surface by the photograph dissolving across the join.
...
- **Output:** The vehicle name renders in amber, because it is the system's answer rather than the user's input. It is honest capacity arithmetic from `src/data/fleet.ts` (`recommendVehicle`) and nothing else. The form's "Type of vehicle" field is a *preference* captured for the operator, deliberately not an input to this line: with one coach in the fleet, arithmetic is the only truthful answer, and a picked option must never be able to make the blade state a vehicle the headcount does not fit.
