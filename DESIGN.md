...
- Footer splits `1fr / 1.45fr` at 60rem; its link columns are `auto-fit, minmax(11rem, 1fr)`. Below 40rem, Contact spans the footer while Site and Services sit in two columns with 44px link targets; the quote action and legal row also take the full available width.
...
- **Fold hazards handled.** A required field inside a closed `<details>` is one the browser cannot focus to report, so an `invalid` listener in the capture phase (the event does not bubble) unfolds the owning `<details>` first. This also covers pressing Enter in step one, which submits implicitly. Opening puts the caret in the first field on roomier layouts; phones keep focus on the disclosure so opening the step does not summon a keyboard over the fields just revealed.
- **Netlify is the delivery layer.** The homepage quote, reservation and contact forms are static HTML forms with unique names, `data-netlify="true"`, and clipped honeypots. Quote and reservation submissions are URL-encoded and POSTed to `/` so their inline blades remain on screen; the Contact form and every no-JavaScript submission POST to `/thanks/`. The wedding package form is deliberately excluded: it is a GET-based planning handoff to `/reserve`, not a completed lead.
- **Grid.** Six tracks at ≥64rem: three short fields to a row, two locations to a row. Four tracks at ≥40rem, two at ≥30rem. Narrow phones stay single-column so native date/time controls and long select values remain usable; larger phones get pairs once each track has enough room.
...
- **It never lies about delivery.** The status line reports the real state of the submission and nothing softer. "Successfully submitted" is printed only when Netlify accepts the URL-encoded POST. A failed POST says so and offers the phone. Editing a field after a send flips the status to "send again to update it", so a revealed blade can never show a route that was never submitted.
...
- **Name and capacity lead on small screens.** Below 60rem, the vehicle name and passenger capacity move above the photography so visitors know what they are looking at before the frame. The first listing shot is always the exterior; mobile and tablet layouts hide every later shot—and the now-redundant `Exterior` caption—so a vehicle does not spend two screens on photography before its specifications. Desktop keeps its specification column and exterior/interior pair, and the individual vehicle gallery remains unchanged at every width.
...
### About page — `/about`
The page is an operator story rather than a generic company biography. It moves through two pieces of proof in order: the exterior with the operator-versus-broker explanation, then the cabin beside the three planning steps. Both photographs already belong to the project; no stock vehicle image is allowed to stand in for the fleet.

- **Desktop alternates copy and photography.** The opening and planning sections use unequal two-column grids. This creates a visual rhythm without introducing card shells around the prose.
- **Mobile becomes a deliberate single-column story.** The opening coordinate and title lead, followed by the first coach and then its supporting copy; the planning copy remains above the interior. The fact rail reduces from four columns to two and then one below 35rem; actions stack at that same narrow breakpoint.
- **Facts stay derived.** Vehicle count and passenger range come from `data/fleet.ts`; address, hours, service reach, the owner-confirmed over-15-years statement and directions come from `config/site.ts`. The page may gain photographs or editorial explanation, but an unconfirmed credential number or coverage amount still belongs in `OPEN_ITEMS` rather than the visible story.

### Close band — `CloseBand.astro`
The site's reservation-focused closing ask, extracted from the homepage when the guides began ending on it. It carries one primary `Reserve now` action to `/reserve`; the footer directly below retains the site's `Get a quote` action, so the two adjacent bands do not repeat the same destination.
